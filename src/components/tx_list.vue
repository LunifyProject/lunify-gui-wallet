  ...
  <div class="tx-list">
    ...
    <template v-else>
      <q-infinite-scroll ref="scroller" @load="loadMore">
        <q-list link no-border :dark="theme == 'dark'" class="lunify-list tx-list">
          <q-item
            v-for="tx in tx_list_paged"
            :key="`${tx.txid}-${tx.type}`"
            class="lunify-list-item transaction"
            :class="'tx-' + tx.type"
...
export default {
  ...
  methods: {
    ...
    txContains(tx, value) {
      ...
      const formattedAmount = tx.amount / 1e8;
