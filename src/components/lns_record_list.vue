<template>
  <div v-if="records.length > 0" class="lns-record-list">
    <div v-if="needsDecryption" class="decrypt q-pa-md row justify-between items-end">
      <LunifyField :label="$t('fieldLabels.decryptRecord')" :disable="decrypting" :error="$v.name.$error">
        <q-input
          v-model.trim="name"
          :dark="theme == 'dark'"
          borderless
    ...
    <q-list link no-border :dark="theme == 'dark'" class="lunify-list">
      <q-item v-for="record in records" :key="record.name_hash" class="lunify-list-item">
