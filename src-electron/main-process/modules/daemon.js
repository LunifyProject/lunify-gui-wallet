...
export class Daemon {
  constructor(backend) {
    ...
    this.PIVOT_BLOCK_HEIGHT = 11730;
    this.PIVOT_BLOCK_TIMESTAMP = 1597688400;
  ...
  checkVersion() {
    return new Promise(resolve => {
      if (process.platform === "win32") {
        let lunifyd_path = path.join(__lunify_bin, "lunifyd.exe");
        let lunifyd_version_cmd = `"${lunifyd_path}" --version`;
        if (!fs.existsSync(lunifyd_path)) {
          resolve(false);
        }
        child_process.exec(lunifyd_version_cmd, (error, stdout) => {
          if (error) {
            resolve(false);
          }
          resolve(stdout);
        ...
        let lunifyd_path = path.join(__lunify_bin, "lunifyd");
        let lunifyd_version_cmd = `"${lunifyd_path}" --version`;
        if (!fs.existsSync(lunifyd_path)) {
          resolve(false);
        }
        child_process.exec(lunifyd_version_cmd, { detached: true }, (error, stdout) => {
          if (error) {
            resolve(false);
          }
          resolve(stdout);
  ...
  start(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      args.push("--log-file", path.join(dirs[net_type], "logs", "lunifyd.log"));
      ...
      portscanner
        ...
        .then(status => {
          if (status === "closed") {
            if (process.platform === "win32") {
              this.daemonProcess = child_process.spawn(path.join(__lunify_bin, "lunifyd.exe"), args);
            } else {
              this.daemonProcess = child_process.spawn(path.join(__lunify_bin, "lunifyd"), args, {
                detached: true
              });
            }
  ...
  _sanitizeLNSRecords(records) {
    return (records || []).map(record => {
      ...
      // Session = 0
      // For now since wallet and lunify names haven't been implemented, we always assume it's session
      const type = "session";
      return {
        ...record,
        type
