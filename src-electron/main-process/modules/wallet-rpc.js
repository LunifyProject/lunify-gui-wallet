  ...
  start(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      crypto.randomBytes(64 + 64 + 32, (err, buffer) => {
        ...
        const rpcExecutable = process.platform === "win32" ? "lunify-wallet-rpc.exe" : "lunify-wallet-rpc";
        // eslint-disable-next-line no-undef
        const rpcPath = path.join(__lunify_bin, rpcExecutable);

        // Check if the rpc exists
        if (!fs.existsSync(rpcPath)) {
          reject(new Error("Failed to find Lunify Wallet RPC. Please make sure you anti-virus has not removed it."));
          return;
        }
  ...
  async handle(data) {
    ...
    switch (data.method) {
      ...
      case "restore_view_wallet":
        // TODO: Decide if we want this for lunify
  ...
  listWallets(legacy = false) {
    ...
    if (legacy) {
      ...
      if (os.platform() == "win32") {
        legacy_paths = ["C:\\ProgramData\\Lunify"];
      } else {
        legacy_paths = [path.join(os.homedir(), "Lunify")];
      }
