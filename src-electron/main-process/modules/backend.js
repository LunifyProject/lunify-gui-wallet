...
export class Backend {
  ...
  init(config) {
    if (os.platform() === "win32") {
      this.config_dir = "C:\\ProgramData\\lunify";
      this.wallet_dir = `${os.homedir()}\\Documents\\Lunify`;
    } else {
      this.config_dir = path.join(os.homedir(), ".lunify");
      this.wallet_dir = path.join(os.homedir(), "Lunify");
    }
  ...
  handle(data) {
    ...
    switch (data.method) {
      ...
      case "open_explorer": {
        ...
        if (path) {
          const baseUrl = net_type === "testnet" ? "" : "https://explorer.lunify.xyz";
